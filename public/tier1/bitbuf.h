...
BITBUF_INLINE void bf_write::WriteUBitLong( unsigned int curData, int numbits, bool bCheckRange ) RESTRICT
{
	...
	unsigned int * RESTRICT pOut = &m_pData[iDWord];
	...
	int i = mask2 & 1;
	unsigned int dword1 = LoadLittleDWord( pOut, 0 );
	unsigned int dword2 = LoadLittleDWord( pOut, i );
...
public:
	...
	BITBUF_INLINE int ReadLong() { return ReadUBitLong(32); }
	int64			ReadLongLong();
	float			ReadFloat();
...
inline int bf_read::ReadOneBitNoCheck()
{
	...
	unsigned int value = ((unsigned int * RESTRICT)m_pData)[m_iCurBit >> 5] >> (m_iCurBit & 31);
#else
...
BITBUF_INLINE unsigned int bf_read::ReadUBitLong( int numbits ) RESTRICT
{
	...
	unsigned int dw1 = LoadLittleDWord( (unsigned int* RESTRICT)m_pData, iWordOffset1 ) >> iStartBit;
	unsigned int dw2 = LoadLittleDWord( (unsigned int* RESTRICT)m_pData, iWordOffset2 ) << (32 - iStartBit);
